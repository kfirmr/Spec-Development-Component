...
- [Entity module](skills/entity-module.md): the canonical structure of a NestJS entity module — the folder tree, the boundaries between controller, service, repository and entity, the rules for each file, and the naming reference. Use it when creating a new entity, adding a file to an existing module, or deciding which layer a piece of server code belongs in.
...
- **Claude** — [CLAUDE.md](CLAUDE.md) consolidates every file in [rules/](rules/) into one document that Claude Code loads as project-wide guidance. Drop it at the root of any repository that should follow these standards. Skills go to `.claude/skills/<name>/SKILL.md` with a `name` and `description`, so Claude Code can invoke them on demand.
...
[rules/](rules/) and [skills/](skills/) are the source of truth; `CLAUDE.md`, `.claude/` and any `.github/` copies are derived from them and must be regenerated when a rule or skill changes.
